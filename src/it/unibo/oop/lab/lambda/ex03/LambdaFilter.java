...
public final class LambdaFilter extends JFrame {
	...
	private enum Command {
		...
		CHARACTERS("Total of chars",
				text -> Integer.toString(text.lines().map(String::length).reduce(0, Integer::sum))),
		LINES("Total of lines", text -> Integer.toString((int) text.lines().count())),
		ALPHABETIC("Words in order",
				text -> Arrays.stream(text.split("\\s+")).sorted().collect(Collectors.joining("\n"))),
		WORDS("Words occasion",
