...
public final class LambdaUtilities {
	...
	public static <T> List<T> dup(final List<T> list, final UnaryOperator<T> op) {
		final List<T> anotherList = new ArrayList<>(list.size() * 2);
		list.stream().forEach(x -> {
			anotherList.add(x);
			anotherList.add(op.apply(x));
	...
	public static <T> List<Optional<T>> optFilter(final List<T> list, final Predicate<T> pre) {
		return list.stream().map(x -> Optional.of(x).filter(pre)).collect(Collectors.toList());
	}
	...
	public static <R, T> Map<R, Set<T>> group(final List<T> list, final Function<T, R> op) {
		 ...
		 */
		return list.stream().collect(Collectors.groupingBy(op, Collectors.toSet()));
	}
	...
	public static <K, V> Map<K, V> fill(final Map<K, Optional<V>> map, final Supplier<V> def) {
		 ...
		 */
		final Map<K, V> anotherMap = new HashMap<>(map.size());
		map.forEach((k, v) -> {
			anotherMap.put(k, v.orElse(def.get()));
		});
		return anotherMap;
	}
